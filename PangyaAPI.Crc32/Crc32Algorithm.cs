...
namespace PangyaAPI.Crc32
{
	...
	public class Crc32Algorithm : HashAlgorithm
	{
        ...
        public static string Compute(FileStream inputStream)
        {
            byte[] array = new byte[4096];
            var Length = inputStream.Read(array, 0, 4096);
            byte[] NewArray = new byte[Length];
			Buffer.BlockCopy(array,0, NewArray, 0 ,NewArray.Length);
			var result = Convert.ToInt64(Append(0, NewArray, 0, Length));
